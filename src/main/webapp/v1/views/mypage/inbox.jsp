	...
	<form method="post" action="deleteInbox.do" onsubmit="return confirmDelete();">
		...
		<c:if test="${not empty messageList}">
			<table class="table-basic">
				<caption>받은쪽지 테이블 목록 - 체크박스, 보낸 사람, 제목, 보낸 날짜, 읽음 여부로 구성</caption>
			    <colgroup>
			        <col style="width:15%">
			        <col style="width:20%">
			        <col style="width:27%">
				...
				<thead>
					<tr>
						<th>
							<label for="checkAll" class="input-check type-only">
		                        <input type="checkbox" id="checkAll" class="form-control" onclick="toggleAll(this)">
				...
				<tbody>
					<c:forEach var="msg" items="${messageList}">
						<tr>
							<td>
								<label for="messageId" class="input-check type-only">
			                        <input type="checkbox" id="messageId" name="messageId" class="form-control"  value="${msg.messageId}">
			                        <span class="checkMark"></span>
			                    </label>
							</td>
							<td>${msg.senderId}</td>
			...
			<div class="paging-area">
			    <ul class="pagination">
				    ...
				    <c:forEach begin="${paging.startPage}" end="${paging.endPage}" var="i">
				      <li class="${i == paging.nowPage ? 'on' : ''}">
					      <a href="inbox.do?page=${i}">${i}</a>
				      </li>
				    </c:forEach>
		...
		<div class="btn-area-right">
			...
			<button type="button" onclick="openComposePopup()" class="btn btn-primaryß">쪽지 쓰기</button>
		</div>
	</form>
  </section>
