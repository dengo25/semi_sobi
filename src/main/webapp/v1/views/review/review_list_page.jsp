...
<%
    ...
    List<ReviewVO> list = (selectedCategoryId > 0) ?
        reviewDao.getReviewByCategory(selectedCategoryId) : reviewDao.getAllReviews();  // ✅ 수정됨
...
<body>
    <div class="container">
        ...
        <div class="category-bar">
            <a href="?categoryId=0" class="${selectedCategoryId == 0 ? 'active' : ''}">전체</a>
            <c:forEach var="category" items="${categoryList}">
                <a href="?categoryId=${category.reviewCategoryId}" class="${selectedCategoryId == category.reviewCategoryId ? 'active' : ''}">
        ...
        <div id="reviewList">
            <c:forEach var="review" items="${reviewList}" varStatus="status">
                <div class="review-card ${status.index >= 10 ? 'hidden' : ''}">
                    <img src="${review.imageUrl}" alt="리뷰 이미지">
                    <div class="review-text">
                        <h4><a href="reviewDetail.do?reviewId=${review.reviewId}">${review.title}</a></h4>
                        <p>작성자: ${review.memberId}</p>
                        <p>평점: ${review.rating}점</p>
                        <p><fmt:formatDate value="${review.createdAt}" pattern="yyyy-MM-dd" /></p>
                    </div>
                </div>
            </c:forEach>
        </div>
